...
Renderer::Renderer(const Frame& f,int w,int h,RenderOptions options):opt_(options){
 ...
 {int tw=512,th=1024;auto* surf=cairo_image_surface_create(CAIRO_FORMAT_ARGB32,tw,th);auto* paint=cairo_create(surf);
  cairo_set_operator(paint,CAIRO_OPERATOR_CLEAR);cairo_paint(paint);cairo_set_operator(paint,CAIRO_OPERATOR_OVER);
  for(int i=0;i<8;i++){
   cairo_set_source_rgba(paint,.008,.018,.027,.99);cairo_rectangle(paint,0,i*128,512,128);cairo_fill(paint);
   cairo_set_source_rgba(paint,.92,.96,.98,.98);text(paint,116,i*128+101,116,"0"+std::to_string(i+1),true);
  }
...
void Renderer::uploadOverlay(const Frame& f,const std::string&) {
 ...
 if(opt_.titles){
  float k=h_/1080.f;double x=46*k,y=48*k;
  // Sparse broadcast graphics leave the characters and circuit in charge.
  cairo_set_source_rgba(c,.005,.012,.031,.57);cairo_rectangle(c,x-15*k,y-25*k,296*k,80*k);cairo_fill(c);
  cairo_set_source_rgba(c,.36,.95,.81,.96);text(c,x,y-2*k,11*k,"QINDAQT   /   AFTERGLOW GRAND PRIX",true);
  cairo_set_source_rgba(c,.94,.97,1,.96);text(c,x,y+30*k,27*k,"PRISM CIRCUIT",true);
  cairo_set_source_rgba(c,.66,.76,.88,.86);text(c,x,y+49*k,10*k,f.courseName+"   /   AUTONOMOUS RACE");
  V3 col=racerColors[f.focus];
  double bottom=h_-95*k;
  cairo_set_source_rgba(c,.005,.012,.03,.65);cairo_rectangle(c,x-15*k,bottom-35*k,290*k,80*k);cairo_fill(c);
  cairo_set_source_rgba(c,col.x,col.y,col.z,.95);cairo_rectangle(c,x-15*k,bottom-35*k,3*k,80*k);cairo_fill(c);
  std::ostringstream name;name<<f.position<<"  /  "<<racerNames[f.focus];
  cairo_set_source_rgba(c,.96,.97,1,.95);text(c,x,bottom-7*k,22*k,name.str(),true);
  std::ostringstream status;status<<"LAP "<<f.lap<<" / 3     "<<int(std::round(f.speed))<<" km/h";
  cairo_set_source_rgba(c,.64,.78,.87,.93);text(c,x,bottom+19*k,12*k,status.str());
  if(f.boost>.05f){cairo_set_source_rgba(c,.26,.81,1,.95);text(c,x+214*k,bottom+19*k,10*k,"BOOST",true);}
  if(f.countdown>0){
   cairo_set_source_rgba(c,.9,.99,1,.94);text(c,w_/2.f-20*k,h_*.23f,58*k,std::to_string(f.countdown),true);
  }
  if(f.winner>=0){
   std::string title=std::string(racerNames[f.winner])+"  /  RACE WINNER";
   cairo_set_source_rgba(c,.92,.8,.42,.97);text(c,w_*.5f-185*k,78*k,23*k,title,true);
  }
  if(!f.map.empty()){
   double mx=w_-131*k,my=h_-124*k;float sz=73*k;
   cairo_set_line_width(c,2*k);cairo_set_source_rgba(c,.23,.36,.51,.72);
   for(size_t i=0;i<f.map.size();i++){
    auto p=f.map[i];if(i)cairo_line_to(c,mx+p.x*sz,my+p.y*sz);else cairo_move_to(c,mx+p.x*sz,my+p.y*sz);
   }cairo_stroke(c);
   for(int i=7;i>=0;i--){V3 co=racerColors[i];cairo_set_source_rgba(c,co.x,co.y,co.z,.95);cairo_arc(c,mx+f.mapX[i]*sz,my+f.mapY[i]*sz,(i==f.focus?4.2:2.7)*k,0,2*pi);cairo_fill(c);}
  }
 }
 if(opt_.titles&&!f.itemLabel.empty()){float k=std::min(h_/900.f,w_/1440.f);cairo_set_source_rgba(c,.32,.95,.8,.95);text(c,w_*.5-64*k,h_-36*k,13*k,f.itemLabel,true);}
