...
versions={'qinda-patrol':'0.2.0','circuit-reef':'1.1.0','prism-circuit':'1.2.0','prism-brawl':'1.2.1','starward-reimagined':'2.1.0'}
 ...
 for project in versions:
  ...
  for sub in ['assets','docs']:
   for source in (root/project/sub).rglob('*'):
    if source.is_file() and (source.suffix in ['.md','.svg','.json'] or
                             (source.suffix=='.png' and 'textures' in source.relative_to(root/project).parts)):
