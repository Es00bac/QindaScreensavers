...
void initialize(Frame& f){
 f.batches[Sphere].mesh=parametric("sphere",36,24,[](float u,float v){float a=u*2*pi,b=v*pi;return V3{std::cos(a)*std::sin(b),std::cos(b),std::sin(a)*std::sin(b)};});
 ...
 f.batches[Torus].mesh=parametric("machined-collar",64,12,[](float u,float v){float a=u*2*pi,b=v*2*pi;return V3{(1+.055f*std::cos(b))*std::cos(a),.055f*std::sin(b),(1+.055f*std::cos(b))*std::sin(a)};},true);
 Mesh cyl=parametric("cylinder",36,1,[](float u,float v){float a=u*2*pi;return V3{std::cos(a),v*2-1,std::sin(a)};},true);
 for(int sign:{-1,1})append(cyl,parametric("",36,1,[=](float u,float v){float a=u*2*pi;return V3{v*std::cos(a),float(sign),v*std::sin(a)};},sign<0));
