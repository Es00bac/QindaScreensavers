...
void exportModels(const std::string& directory) {
    ...
    for (int asset = 0; asset < 14; ++asset) {
        for (auto& batch : frame.batches) batch.instances.clear();
        std::string name;
        switch (asset) {
        case 0:
            name = "cyberpengu-and-courier";
            ship(frame, M4::identity(), false, 4);
            break;
        case 1:
            name = "ducke-and-scout";
            ship(frame, M4::identity(), true, 4);
            break;
        case 2:
            name = "ember-dock";
            station(frame, M4::identity(), 0, false);
            break;
        case 3:
            name = "sleeping-archive";
            gate(frame, M4::identity(), 0, 1);
            break;
        case 4:
            name = "freighter-08";
            freighter(frame, M4::identity(), 0, 1);
            break;
        case 5:
            name = "sentinel";
            sentinel(frame, M4::identity(), 0);
            break;
        case 6:
            name = "signal-capsule";
            capsule(frame, M4::identity(), 0);
            break;
        case 13:
            name = "starlight-sail";
            creature(frame, M4::identity(), 0);
            break;
        default:
            name = "asteroid-" + std::to_string(asset - 7);
            asteroid(frame, M4::identity(), asset - 7);
            break;
        }
        ...
        obj << std::setprecision(7)
            << "# Starward Reimagined / GPL-3.0-or-later\nmtllib "
            << name << ".mtl\ns 1\n";
