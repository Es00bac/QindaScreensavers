...
void main(){
 ...
 vec3 center=vec3(165,64,-540);float radius=110.;vec3 oc=eye-center;
 ...
 if(abs(den)>.001){float t=dot(center-eye,rn)/den;if(t>0&&t<planetT){
  ...
  if(d>130&&d<174){float ring=.12+.10*pow(.5+.5*sin(d*2.7),3.);col=mix(col,vec3(.19,.14,.32)+vec3(.20,.32,.42)*ring,smoothstep(130,134,d)*(1-smoothstep(168,174,d))*.67);}
 }}
